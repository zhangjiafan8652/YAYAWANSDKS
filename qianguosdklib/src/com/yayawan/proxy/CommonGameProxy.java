...
public class CommonGameProxy implements YYWGameProxy {
	...
	public void pay(final Activity paramActivity, final YYWOrder order,
		...
		/*game_id int No 游戏id 
		uid string No 用户注册返回的唯一id 
		union_id int No 联合渠道编号 
		username string No 用户名 
		osversion int Yes 系统版本 
		appversion int Yes app版本 */

		
		
		
		
		Handle.pay_handler(paramActivity, DeviceUtil.getGameId(paramActivity), YYWMain.mUser.uid, DeviceUtil.getUnionid(paramActivity),
				YYWMain.mUser.userName, getAndroidOSVersion()+"", AppUtils.getAppVersionCode(paramActivity)+"", new YYWLoginHandleCallback() {
					...
					public void onSuccess(String response, String temp) {
						...
						try {
							...
							if (success==1) {
								...
								goPay(paramActivity, order, YYWMain.mPayCallBack , loca_login_type);
							}else {
								int login_pay=jsonObject.optInt("login_pay");
								Yayalog.loger("支付时候loca_login_type：" + login_pay);
								goPay(paramActivity, order, YYWMain.mPayCallBack , login_pay);
							}
						} catch (JSONException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
