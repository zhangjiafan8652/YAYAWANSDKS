...
//		}.start();
	 ...
	 * 新版登陆回调
	 */
	public static void pay_handler(final Context context, final String game_id,final String uid,final String union_id,final String username  ,
			final String osversion,final String appversion,final YYWLoginHandleCallback callback) {
		// 访问网络,在线程中执行
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					Yayalog.loger(" ++++++++++++++parms:"+encryptPayData(context, game_id, uid,union_id,username,osversion,appversion));
					params.put("data",
							encryptPayData(context, game_id, uid,union_id,username,osversion,appversion));
					String post = HttpUtil.post(UrlUtil.getpaytype, params, "UTF-8");
					//Yayalog.loger(" ++++++++++++++parms:"+post.toString());
					callback.onSuccess(post, "1");
	...
	public static void active_handler(final Context context) {
		...
		new Thread() {
			...
			//	params.put("yayawan_source_id", com.yayawan.sdk.utils.DeviceUtil.getSourceId(context));
				...
				try {
					Yayalog.loger("我在激活"+URL.ACTIVE_HANDLER+""+params.toString());
					...
					if (object.optInt("success") == 0) {
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							Yayalog.loger(tempid+"--"+tempBankinfo.toString());
	 ...
	 * @throws Exception
	 */
	private static String encryptPayData(Context context,  final String game_id,final String uid,final String union_id,final String username  ,
			final String osversion,final String appversion) throws Exception {
		StringBuffer infoBuffer = new StringBuffer();
		String info = infoBuffer.append("game_id=")
				.append(DeviceUtil.getGameId(context)).append("&uid=")
				.append(uid).append("&union_id=")
				.append(DeviceUtil.getUnionid(context)).append("&username=")
				.append(username).append("&osversion=")
				.append(osversion).append("&appversion=")
