...
public class Yayapay_mainxml_po extends Basexml implements Layoutxml {
	...
	public View initViewxml() {
		...
		TextView tv_fastpay = markView("请选择支付方式：如支付失败，请换种支付方式");
		
		//创建每种支付的布局
		rl_mAlipay = createItemView("支付宝支付", "yaya_zhifu.png",DeviceUtil.ALIPAYMSPCODE);
		rl_mYinlianpay = createItemView("银联卡支付", "yaya_yinlian.png",DeviceUtil.YINLIAN);

		rl_mChuxuka = createItemView("储蓄卡支付", "yaya_chuxuka.png",DeviceUtil.YIBAOCODE);
		rl_mXinyongka = createItemView("信用卡支付", "yaya_xinyongka.png",DeviceUtil.YIBAOCODE);
		rl_mYidong = createItemView("移动充值卡", "yaya_yidong.png",DeviceUtil.YIDONGCODE);

		rl_mLiantong = createItemView("联通充值卡", "yaya_liantong.png",DeviceUtil.LIANTONGCODE);
		rl_mDianxin = createItemView("电信充值卡", "yaya_dianxin.png",DeviceUtil.DIANXINCODE);
		rl_mShengda = createItemView("盛大充值卡", "yaya_shengda.png",DeviceUtil.SHENGDACODE);

		rl_mJunka = createItemView("骏卡一网通", "yaya_junka.png",DeviceUtil.JUNWANGCODE);
		rl_mYaya = createItemView("丫丫币充值", "yaya_yayabi.png",DeviceUtil.YAYABICODE);
		rl_mQbi = createItemView("QQ卡充值", "yaya_qqpay.png",DeviceUtil.QQCODE);

		rl_mWxpay = createItemView("微信支付", "yaya_weixinpay.png",DeviceUtil.WXPAYCODE);
		
		rl_mWxpluin = createItemView("安全微支付", "yaya_weixinpay.png",DeviceUtil.YAYAWANWEIXINPLUIN);
		// TODO
		...
		bt_mMorepay.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (mpaytostring.contains("yaya_wxpluin")) {
	 ...
	 * @param paytype   支付方式id   用来判断是否九五折
	 * @return
	 */
	private RelativeLayout createItemView(String name, String iconname, int paytype) {
		...
		String tempdiscount = ViewConstants.banks.get(paytype).getDiscount()+"";
		//Yayalog.loger("创建支付页面时候打的折："+tempdiscount);
		Yayalog.loger("创建支付页面时候打的折:"+ViewConstants.banks.get(paytype));
		if (ViewConstants.banks.get(paytype).getDiscount()<100&&ViewConstants.banks.get(paytype).getDiscount()>2) {
			char[] charArray = tempdiscount.toCharArray();
			
			tempdiscount=charArray[0]+"."+charArray[1];
			//Integer.parseInt(tempdiscount+"")
			String tempname = name+tempdiscount+"折";
			SpannableStringBuilder style=new SpannableStringBuilder(tempname); 
			
			style.setSpan(new ForegroundColorSpan(Color.RED),name.length(),tempname.length(),Spannable.SPAN_EXCLUSIVE_INCLUSIVE); //设置指定位置文字的颜色
			machineFactory.MachineTextView(tv_alipay, WRAP_CONTENT, MATCH_PARENT,
					0, name, 22, mRelativeLayout, 90, 0, 0, 0);
			tv_alipay.setText(style);
		}else {
			name=name+"      ";
			machineFactory.MachineTextView(tv_alipay, WRAP_CONTENT, MATCH_PARENT,
					0, name, 22, mRelativeLayout, 90, 0, 0, 0);
