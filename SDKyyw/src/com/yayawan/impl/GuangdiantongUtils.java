...
public class GuangdiantongUtils {
	...
	public static void guangDiantongInit(Context mcontext) {
		...
		if (!sPstring.equals("no")) {
			 ...
			 *            applicationContext
			 * @param channel
			 *            渠道标识，注意：目前仅支持传入"TrackConstants.APP_CHANNEL"类中预定义的渠道标识，
			 *            传入其他字符串代表的渠道将无法被统计。
			 */
			GDTTracker.init(mcontext, TrackConstants.APP_CHANNEL.UNION_APP);
	...
	public static void guangDiantongRegister(Context mcontext) {
		...
		if (!sPstring.equals("no")) {
			...
			GDTTracker.logEvent(mcontext,
					TrackConstants.CONVERSION_TYPE.REGISTER);
		}
	...
	public static void guangDiantongGiveMoney(Context mcontext, String money) {
		...
		if (!sPstring.equals("no")) {
			...
			GDTTracker.logEvent(mcontext,
					TrackConstants.CONVERSION_TYPE.PURCHASE,
	 ...
	 * @return
	 */
	public static void getGuangdiantong(final Context mcontext,
			final String conv_type) {
		...
		String advertiser_id = DeviceUtil
				.getGameInfo(mcontext, "advertiser_id");
		...
		String pingjieUrl = pingjieUrl(qqappid, muid, startPaintLogoTime + "",
				null, sign_key, conv_type, "UNIONANDROID", advertiser_id,
				encrypt_key);
		// return pingjieUrl;

		HttpUtils httpUtils = new HttpUtils();
		httpUtils.send(HttpMethod.GET, pingjieUrl,
		 ...
		 * 
		 * @Override public void sucee(String str) { // TODO Auto-generated
		 * method stub System.out.println("广点通上报数据返回结果"+str); try { JSONObject
		 * jsonObject = new JSONObject(str); int int1 =
		 * jsonObject.getInt("ret"); if (int1==0) {
		 * Sputils.putSPstring(conv_type, "yes", mcontext);
		 * System.out.println("广点通激活成功"+str); } } catch (JSONException e) { //
		 * TODO Auto-generated catch block e.printStackTrace(); } }
		 * 
		 * @Override public void faile(String err, String rescode) { // TODO
		 * Auto-generated method stub //System.out.println("广点通上报数据失败返回结果"+err);
		 * 
	 ...
	 *            平台提供的sign_key
	 * @param conv_type
	 *            上报什么数据 激活 MOBILEAPP_ACTIVITE 注册 MOBILEAPP_REGISTER 加入购物车
	 *            MOBILEAPP_ADDTOCART 付费行为 MOBILEAPP_COST
	 * @param apptype
	 *            填入 ANDROID
	 * 
	 * @param advertiser_id
	...
	public static String pingjieUrl(String appid, String muid,
			...
			String conv_type, String apptype, String advertiser_id,
			String encrypt_key) {
		String query_string;
