...
/**
 * 接入千果只需九步哦...复制粘贴几步搞点 1.sdk初始化方法。
 * 
 * YaYaWan.getInstance().initSdk(mActivity);
 * 
 * 2.复写生命周期方法,里面调用 YaYaWan.getInstance().onCreate(this);
 ...
 * 
 * 3.闪屏动画,在游戏启动时的第一个页面出现..只要保证游戏画面出来前调用即可 YaYaWan.getInstance().anim(this, new
 * YYWAnimCallBack(){})
 * 注意:闪屏回调有三种情况..不管闪屏失败还是成功.都应该进入游戏画面..也就是说,不管闪屏失败还是成功.游戏继续进行
 * 
 * 4.登录接口. YaYaWan.getInstance().login(this, new YYWUserCallBack()} 如下面的例子:
 * 注意:登录有三个回调..第一个成功,第二个取消...注意第三个是登出..登出则是用户点击了sdk中的切换账号按钮,游戏应该注销账户 回到游戏的登录页面..
 * 
 * 5.设置角色数据 YaYaWan.getInstance().setRoleData(MainActivity.this, user.uid,
 * user.userName, "11", "1", "无尽之海"); 注意:在角色登录成功后调用,上传角色信息
 * 
 * 6.支付接口 YaYaWan.getInstance().pay(this, order, new YYWPayCallBack()) 看例子:
 * 
 * 7.退出接口 YaYaWan.getInstance().exit(this, new YYWExitCallback() )
 * 
 * 
 * 
 * 8.游戏内注销按钮...(可选)应用场景,游戏内有注销账号的按钮..点击按钮后游戏自己退出游戏，在退出之前调用这个接口（这个接口无任何现象，
		...
		YaYaWan.getInstance().login(this, new YYWUserCallBack() {
			...
			public void onLoginSuccess(YYWUser user, Object arg1) {
				...
				Toast.makeText(GameActivity.this, "登录成功" + user,
						Toast.LENGTH_SHORT).show();
				// 登录成功后设置角色数据
				YaYaWan.getInstance().setRoleData(GameActivity.this, user.uid,
