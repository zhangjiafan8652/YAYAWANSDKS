...
import com.yayawan.sdk.jfpay.WeiXinZhiFuBaoPay;
import com.yayawan.sdk.jfpay.XiaoMipayActivity;
...
public class CommonGameProxy implements YYWGameProxy {
	...
	public void login(final Activity paramActivity,
		...
		// 判断本地登陆支付
		loca_login_type = Sputils.getSPint("loca_login_type", 4, paramActivity);
		...
		YYWMain.mUserCallBack = new YYWUserCallBack() {
			...
			public void onLoginSuccess(final YYWUser paramUser,
				...
				// String qudaouid=paramUser.uid;
				// String qudaousername=paramUser.userName;
				//
				// String qudaotoken=paramUser.token;
				//
	...
	public void pay(final Activity paramActivity, final YYWOrder order,
		...
		String temppay = "no";
		try {
			temppay = DeviceUtil.getGameInfo(paramActivity, "USE_YAYAPAY");
		} catch (Exception e) {
			// TODO Auto-generated catch block
		...
		/*
		 * game_id int No 游戏id uid string No 用户注册返回的唯一id union_id int No 联合渠道编号
		 * username string No 用户名 osversion int Yes 系统版本 appversion int Yes
		 * app版本
		 */

		Handle.pay_handler(paramActivity, DeviceUtil.getGameId(paramActivity),
				YYWMain.mUser.uid, DeviceUtil.getUnionid(paramActivity),
				YYWMain.mUser.userName, getAndroidOSVersion() + "",
				AppUtils.getAppVersionCode(paramActivity) + "",
				new YYWLoginHandleCallback() {

					@Override
					public void onSuccess(String response, String temp) {
						// TODO Auto-generated method stub
						Yayalog.loger("支付返回：" + response);
						try {
							JSONObject jsonObject = new JSONObject(""
									+ response);
							int success = jsonObject.optInt("success");
							if (success == 1) {
								int loca_login_type = Sputils.getSPint(
										"loca_login_type", 4, paramActivity);
								Yayalog.loger("支付时候loca_login_type："
										+ loca_login_type);
								goPay(paramActivity, order,
										YYWMain.mPayCallBack, loca_login_type);
							} else {
								int login_pay = jsonObject.optInt("login_pay");
								Yayalog.loger("支付时候loca_login_type："
										+ login_pay);
								goPay(paramActivity, order,
										YYWMain.mPayCallBack, login_pay);
							}
						} catch (JSONException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
	...
	private void goPay(Activity paramActivity, YYWOrder order,
			YYWPayCallBack payCallBack, int loca_login_type) {
		switch (loca_login_type) {
		case 0:
			this.mCharger = new ChargerImpl();
			this.mCharger.pay(paramActivity, order, payCallBack);
		...
		case 2:
			...
			break;
		case 3:
			String login_pay_level = Sputils.getSPstring("login_pay_level",
					"0", paramActivity);
