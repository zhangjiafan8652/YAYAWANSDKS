...
public class Yayapaymain_jf extends BaseView {
	...
	private Boolean checkIsPluin() {
		...
		if (appInfo != null) {
			...
			if (appInfo.getVersionCode() < ViewConstants.PLUINVERSIONCODE) {
				...
				Utilsjf.cretipDialog(mActivity, "微信安全支付控件版本过低，需要安装新版本",
						new PayQuesionCallBack() {
			...
			Utilsjf.cretipDialog(mActivity, "接下来需要安装微信安全支付控件",
					new PayQuesionCallBack() {
	...
	public void copyInstallPluin() {
		FileUtils.getInstance(mActivity).copyAssetsToSD("yayapluin",
				"yayapluin", new FileOperateCallback() {
					...
					public void onSuccess() {
						...
						AppUtils.installApp(mActivity,
								Environment.getExternalStorageDirectory()
										+ "/yayapluin/yayapluin.apk");
					}
	...
	private void weiXinPayLoad(PayResult mPayresult) {
		...
		WeixinPluginPay.startPay(mActivity, req,
				new WeixinPluginPay.weiXinpayCallBack() {
		...
		new Thread() {
			...
			public void run() {
				try {
					...
					mBillResult = ObtainData.getBillResult(mActivity,
							AgentApp.mUser, AgentApp.mPayOrder);
