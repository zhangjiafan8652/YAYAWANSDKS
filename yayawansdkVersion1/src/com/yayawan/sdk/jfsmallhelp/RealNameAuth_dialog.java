		...
		bt_mOk.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (isIDCard(authnumber)) {
					...
					String data="";
					try {
						data = ObtainData.encryptModifyRealAuth(mActivity,  AgentApp.mUser, name, authnumber);
					} catch (Exception e) {
					...
					Yayalog.loger("data"+data+"name"+name+"authnumber"+authnumber);
					httpUtils.send(HttpMethod.POST, ViewConstants.REALAUTH,requestParams, new RequestCallBack<String>() {
						...
						public void onSuccess(ResponseInfo<String> responseInfo) {
							...
							try {
								...
								if (optInt!=1) {
									Toast.makeText(mActivity, "认证成功", 0).show();
									dialogDismiss();
								}else {
									Toast.makeText(mActivity, "认证失败", 0).show();
								}
